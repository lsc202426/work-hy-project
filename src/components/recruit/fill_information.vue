					...
					<div class="voucher-center">
						<div class="voucher-case" v-for="(item, index) in imgArr" :key="index">
							<div class="img_minus setDelBtn-img-hook" v-show="imgArr.length">
								<div class="img-voucher" v-bind:style="{
                                        backgroundImage: 'url(' + 'http://oapi.huyi.cn:6180/' + item.fileurl + ')',
                                    }"></div>
							</div>
							<!-- 删除的小图标 -->
							<img src="../../assets/images/user/icon_remove.png" class="del-icon setDelBtn-el-hook" v-show="imgArr[0]" @click="del_img($event, index, 'imgArr')" />
						</div>
						<!-- 默认图片 -->
			...
			<div class="apply-word" v-if="pageNum == 2">
				...
				<div class="apply-msg">
					...
					<div class="apply-materials-list-img">
						<a href="javascript:void(0);" v-for="(list, i) in imgArr" :key="i" v-bind:style="{
	...
	export default {
		...
		created() {
			...
			if (sessionStorage.proEditId && sessionStorage.mark == "dzp") {
				...
				//获取申请信息
				that.$axios.post("/index.php?c=App&a=getWishlistItem", {
						id: id
					})
					.then((res) => {
