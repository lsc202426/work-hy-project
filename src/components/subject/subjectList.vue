	...
	<div class="subjectList head_box">
		...
		<div class="subject_list_box containerView-main" v-if="lists && lists.length > 0">
			<div class="subject_con">
				...
				<div class="exp_txt">
					按照工信部要求，域名申请人必须实名，若未实名会
					被注册局暂停解析（Serverhold），请及时实名
				</div>
				<div class="subject_item" v-for="(list, index) in lists" :key="list.id" @click="editDetail(list.id, index)">
					<div class="subject_item_list">
						<span>申请人</span>
						<span>{{ list.corpname }}
							<img v-if="list.status == 0||list.status == -1" @click.stop="verifyReal(list.id)" class="icon_verify_go" src="../../assets/images/subject/icon_real_go.png"
							 alt="" />
							<i v-if="list.status" class="list_status" :class="{red:list.status== -1||list.status==0}">{{list.status_name}}</i>
						</span>
					</div>
					<div class="subject_item_list">
						<span>联系人</span>
					...
					<div class="subject_item_list">
						...
						<span>
							...
							<img v-if="list.verify_email == 0" @click.stop="verifyEmail(list.id, list.email)" class="icon_verify_go" src="../../assets/images/subject/icon_verify_go.png"
							 alt="" />
							<i v-if="list.verify_email" class="list_status" :class="{red:list.status== -1||list.status==0}">{{list.verify_email_name}}</i>
							<!-- <img
	...
	export default {
		...
		computed: {
			...mapGetters([
				...
				[GetterTypes.GET_TMD_APPLY_INFO]
			]),
			...mapGetters({
		...
		methods: {
			...mapMutations([
				...
				[MutationTypes.SET_TMD_APPLY_INFO]
			]),
			...mapMutations({
