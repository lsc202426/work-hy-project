	...
	<div class="subjectList">
		...
		<div class="subject_list_box containerView-main" v-if="lists && lists.length>0">
			<div class="subject_con">
				...
				<div class="subject_item" v-for="list in lists" :key="list.id" @click="editDetail(list.id)">
					...
					<div class="subject_item_list">
						<span>审核状态</span>
						<span :class="{status_d:list.status_name=='待审核',status_y:list.status_name=='已审核',status_w:list.status_name=='未通过'}">{{list.status_name}}</span>
					</div>
				</div>
			</div>
		</div>
