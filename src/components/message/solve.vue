            ...
            <div class="solve_list" v-for="list in lists" :key="list.id">
                ...
                <div class="list_content">
                    <div class="img_box" id="img_box">
                        <div
                            class="img_list"
                            v-for="(item, index) in list.data_attachments.split(';').length"
                            :key="index"
                            :style="{
                                backgroundImage: 'url(' + configs.api.public_domain + list.data_attachments.split(';')[index] + ')',
                            }"
                        >
                            <input
                                class="file_i"
                        ...
                        <div class="img_list" v-if="list.data_attr == 2 && list.data_attachments.split(';').length < 2">
                            <input
                                ...
                                accept="image/*"
                                ref="file"
                                @change="fileSelect($event, list.data_attr, list.data_name, list.id)"
                            />
                            <div class="upFile">
                                <div class="add">+</div>
                                <div>上传</div>
    ...
    methods: {
        ...
        fileSelect(even, attr, name, id, attachments, index) {
            ...
            reader.onload = function() {
                let imgcode = reader.result.replace(/^data:image\/(jpeg|png|gif|jpg|bmp);base64,/, '');
            ...
            _this.$axios
                ...
                .then(function(response) {
                    if (response.data.errcode == 0) {
                        ...
                        if (_this.filesAll.length > 0) {
                            //判断要上传的内容是否有
                            //判断是否存在相同项
                            let len = _this.filesAll.length;
                            for (let i = 0; i < len; i++) {
                                if (_this.filesAll[i].id == filesO.id) {
                                    //判断是否存在相同id
                                    if (attr == 1) {
                                        //判断是否单文件类型，
                                        //如果是直接替换
                        ...
                        if (attr == 1) {
                            even.target.parentNode.style.backgroundImage = 'url(' + _this.configs.api.public_domain + _this.imagesUrl + ')';
                        }
                        if (attr == 2) {
                            //如果是多文件上传
                            if (attachments) {
                                even.target.parentNode.style.backgroundImage =
                                    'url(' + _this.configs.api.public_domain + _this.imagesUrl + ')';
                            } else {
                                div = document.createElement('div');
                                div.innerHTML =
                                    '<img src="' +
                                    this.result +
                                    '" style="width:100%;position:absolute;top:0;left:0;right:0;bottom:0;margin:auto;" />';
                                div.style.cssText = 'position: absolute;top:0;left:0;bottom:0;right:0;margin:auto;';
                                // div.classList.add('addImg');
                                even.target.parentNode.appendChild(div);
                                let newEven = even.target.parentNode.cloneNode(true);
                                even.target.parentNode.parentNode.appendChild(newEven);
