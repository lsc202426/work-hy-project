<template>
	<div class="solve">
		...
		<div class="solve_box">
			...
			<div class="solve_list" v-for="(list,index) in lists" :key="list.id">
				<div class="list_title">{{list.data_name}}</div>
				<div class="list_content">
					<div class="img_box" id="img_box">
						<div class="img_list" v-for="(item,index) in list.data_attachments.split(';').length" :style="{backgroundImage: 'url('+'http://oapi.huyi.cn:6180/'+ list.data_attachments.split(';')[index] + ')' }">
							<input class="file_i" type="file" multiple accept='image/*' ref="file" @change="fileSelect($event,list.data_attr,list.data_name,list.id)">
							<div class="upFile">
								<div class="add">+</div>
								<div>上传</div>
							</div>
		...
		methods: {
			...
			init() {
				...
				if (localStorage.msgId) {
					...
					this.$axios.post('index.php?c=App&a=getProblem', {
							userid: 1,
							msid: _this.msid
						})
				...
				if (fileList) {
					...
					for(var i=0;i<fileList.length;i++){
						...
						reader.onload=function(e){
							...
							div.innerHTML='<img src="'+this.result+'" style="width:100%;position:absolute;top:0;left:0;right:0;bottom:0;margin:auto;" />';
							div.style.cssText="position: absolute;top:0;left:0;bottom:0;right:0;margin:auto;z-index:-1;";
			...
			//上传文件处理，获取url
			imgUp(imgResult,name,id){
				let _this=this;
				this.$axios.post('index.php?c=App&a=uploadAttachment', {
						userid: 1,
						filename:name,
						...
						if (response.data.errcode == 0) {
							...
							for(var i=0;i<_this.files.length;i++){
								if(filesO.id==_this.files[i].id){
									_this.files[i].attachments=_this.files[i].attachments+";"+filesO.attachments;
									return;
								}
							}
			...
			upload(){
				let _this=this;
				if(this.files){
					this.$axios.post('index.php?c=App&a=setProblemAttachment', {
							userid: 1,
							msid: _this.msid,
			...
			delImg: function(id,att,event) { //刪除上已选图片的方法
				let _this=this;
				if(_this.files.length){
					for(let i=0;i<_this.files.length;i++){
						if(id==_this.files[i].id){//判断是否存在这个id
							let item=_this.files[i].attachments.split(";");
							if(item.length<2){//判断是否有多个
								//没有直接删除
