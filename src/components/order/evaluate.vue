                ...
                <div class="evaluate_box">
                    ...
                    <div class="bruce flex-ct-x">
                        <div class="star-rating">
                            <span
                                :class="{ active: index < list.star }"
                                type="radio"
                                v-for="(item, index) in 5"
                                :key="index"
                                @click="evaluateI(index, list)"
                                name="rate"
                            ></span>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        ...
        init() {
            ...
            _this.$axios
                .post('index.php?c=App&a=getEvaluateIndex', {
        ...
        release() {
            ...
            if (toastTxt) {
                ...
                this.$axios
                    .post('index.php?c=App&a=setEvaluate', {
        ...
        toBase64(e, list, i) {
            ...
            reader.onload = function() {
                let imgcode;
                if (i && i === 'video') {
                    imgcode = this.result.replace(/^data:video\/(mp4|MPEG4|WebM|ogg);base64,/, '');
                } else {
                    imgcode = this.result.replace(/^data:image\/(jpeg|png|gif|jpg|bmp);base64,/, '');
                }
                _this.$axios
