<template>
  <div class="shoppingCart">
    <nav-header title="已选清单" gobackurl="/user"></nav-header>
    <!-- 内容 -->
    <div
      class="cart_content containerView-main"
    ...
    >
      ...
      <div class="cart_list">
        <div class="cart_item" v-for="(list, index) in lists" :key="index">
          ...
          <div class="icon_left">
            <img
              v-if="list.product_mark == 'domain'"
              src="../../assets/images/shoppingCart/icon_ym.png"
              alt=""
            /><!-- 域名服务domain -->
            <img
              v-if="list.product_mark == 'tmd'"
              src="../../assets/images/shoppingCart/icon_dsb.png"
              alt=""
            /><!-- 点商标tmd -->
            <img
              v-if="list.product_mark == 'dzp'"
              src="../../assets/images/shoppingCart/icon_dzp.png"
              alt=""
            /><!-- 点招聘dzp -->
            <img
              v-if="list.product_mark == 'bs'"
              src="../../assets/images/shoppingCart/icon_sb.png"
              alt=""
            /><!-- 商标服务bs -->
            <img
              v-if="list.product_mark == 'ecweb'"
              src="../../assets/images/shoppingCart/icon_yzt.png"
              alt=""
            /><!--ecweb 一站通 -->
          </div>
          <!-- 右上角删除 -->
          <div class="icon_delete" @click="deleteItem(list.id, list.total)">
            <img
              src="../../assets/images/shoppingCart/icon_delete.png"
              alt=""
            />
          </div>
          <!-- 复选框 -->
          <div class="item_left" :class="{ active: ids.indexOf(list.id) >= 0 }">
            <label for="" class="item_checkbox">
              <input
                ...
                :value="check"
                @click="checkItem(list.id, list.subject.name, list.total)"
              />
            </label>
          </div>
          <!-- 内容 -->
          <div class="item_right">
            ...
            <p v-if="list.product_mark == 'tmd'" class="item_category">
              类别:<span @click="getCategory(list.id)" class="category"
                ><span
                  v-for="(details, index) in list.class_detail"
                  :key="index"
                  >{{ details.categoryName
                  }}<span v-if="details.length > 1">,</span></span
                ><i class="icon_b"></i
              ></span>
            </p>
            ...
            <p class="item_total" @click="getTotal(list.id)">
              合计:￥{{ list.total
          ...
          <transition name="fade" mode="out-in">
            <div
              class="total_detail"
              v-if="price_detail == list.id && list.product_mark == 'tmd'"
            >
          ...
          <transition name="fade" mode="out-in">
            <div
              ...
              v-if="category_detail == list.id && list.product_mark == 'tmd'"
            >
              ...
              <div class="detail_con">
                ...
                <div class="category_con">
                  ...
                  >
                    ...
                    <div
                      class="products_box"
                      v-for="products in details.detail"
                      :key="products.code"
                    >
                      <div
                        class="category_item_con"
